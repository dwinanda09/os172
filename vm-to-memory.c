...
//printf("addrint %d",adressInt);
char *baseAddress = malloc(sizeof(char) * 8);
//char temp[9];

//char *baseAddress = "00200000";



int p1, real_p2,p2,real_offset, offset;

p1 = adressInt >> 22;
real_p2 = adressInt << 10;
real_p2 = real_p2 >> 22;
real_offset = adressInt << 20;
real_offset = real_offset >> 20;


// A1
int A1 = hexstr2int("00200000") + (p1 << 2);
int2hexstr(A1, baseAddress);
//printf("%s\n", baseAddress);
char* entry = load_8bytes_from_memory (baseAddress);
//printf ("%s\n", entry);

// A2
adressInt = hexstr2int(entry);

//p1 = adressInt >> 22;
//p2 = adressInt << 10;
//p2 = p2 >> 22;
int tanpaOffset = adressInt >> 12;
tanpaOffset = tanpaOffset << 12;
offset = adressInt << 20;
offset = offset >> 20;
 
int A2 = tanpaOffset + (real_p2 << 2);
int2hexstr(A2, baseAddress);
entry = load_8bytes_from_memory (baseAddress);
//printf("%s\n", entry);

int real_entry = hexstr2int (entry);

real_entry = real_entry >> 12;
real_entry = real_entry << 12;


// A3
int2hexstr(real_offset, baseAddress);
//printf("real_offset : %s\n", baseAddress); 

int A3 = real_entry + real_offset;
...
/*
char *p01,*p02,*offset01;
int2hexstr(p1,p01);
int2hexstr(p2,p02);
int2hexstr(offset,offset01);
printf("%s %s %s", p01,p02,offset01);
*/
